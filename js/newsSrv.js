angular.module("newsHub").service("newsSrv", function($http) {
  ...
  this.getSources = function(selected) {
    var arr = [];
    return $http
      .get("https://newsapi.org/v1/sources?category=&sortyby=latest")
  ...
  this.getArticles = function(selectedChoices) {
    var promises = [];
    selectedChoices.forEach(function(value) {
      promises.push(
        $http.get("https://newsapi.org/v1/articles?source=" + value.id + key)
      );
    });
    return Promise.all(promises).then(function(response) {
      var articles = [];
      response.forEach(function(cur) {
        cur.data.articles.forEach(function(cur2) {
          articles.push(cur2);
        });
      });
      return articles;
